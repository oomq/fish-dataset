...
result1 = cv2.dilate(canny,cross)
result2 = cv2.erode(result1, diamond)
cv2.imshow('cross', result1)
cv2.imshow('diamond', result2)
...
diamond = cv2.getStructuringElement(cv2.MORPH_RECT,(5, 5))
result2 = cv2.erode(result1, diamond)
cv2.imshow('canny-result', result2)
cv2.waitKey(0)
cv2.destroyAllWindows()
...
canny_mat=canny_mat.astype(np.uint8)
...
plt.imshow(canny_labels)##分割后的两条鱼
plt.show()
mask_label=canny_labels.copy()
mask_label[mask_label > 1] = 0###把2 改成1
mask_label=mask_label.astype(np.uint8)
cross = cv2.getStructuringElement(cv2.MORPH_CROSS,(7, 7))
mask_label_cross = cv2.dilate(mask_label,cross).astype(np.int8)

labels=labels.astype(np.int8)

other = labels-mask_label_cross#用原连通域减去已经匹配好的连通域
other=other.astype(np.int8)
other[other < 1] = 0
...
cv2.imwrite("masked_pic.jpg",masked_pic)
cv2.imwrite("masked_pic2.jpg",masked_pic2)
