...
path_input = 'test-test.xlsx'
result_path = 'output_pic'
    ...
    def execute(self):
        ...
        for x in range(0, 8):
            ...
            for y in range(0, 12):
                ...
                #
                if y == 10:
                    self.exp_tailbeat.append(table_z.col_values(y, 1, self.twohours - 1)
                                       + table_x.col_values(y, 1, self.twentyfour - 1)
                                       + table_j.col_values(y, 1, self.twentyfour - 1)
                                       )

                if y ==11:
                    self.exp_tailbeatfudu.append(table_z.col_values(y, 1, self.twohours - 1)
                                       + table_x.col_values(y, 1, self.twentyfour - 1)
                                       + table_j.col_values(y, 1, self.twentyfour - 1)
                                       )
                if y == 8:
                    self.exp_swr.append(table_z.col_values(y, 1, self.twohours - 1)
                                       + table_x.col_values(y, 1, self.twentyfour - 1)
                                       + table_j.col_values(y, 1, self.twentyfour - 1))
        ...
        self.plot_tailbeat(y, x, heading)
        self.plot_tailbeatfudu(y, x, heading)
        self.xlsoutput.close()

    def plot_eight(self, y, x, heading):###转弯
        ...
        plt.legend(loc='upper left',labels=exp_ind,markerscale=10)
        ...
        plt.savefig(result_path + '/turn.png')

    def plot_tailbeat(self, y, x, heading):
        n = len(self.exp_tailbeat[0])
        fig_turn = plt.figure()
        ax5 = fig_turn.add_subplot(1, 1, 1)
        ax5.set_title("tail-beat frequency")
        ax5.set_xlabel('hours')
        ax5.set_ylabel("avg_s_times")
        plt.xlim(-1.5, 40)
        # plt.ylim(0, y_zhou[5])
        x_zhou = np.linspace(-1.5, 40, n).tolist()
        for ind, data in enumerate(self.exp_tailbeat):
            # print(color[ind])
            ax5.scatter(x_zhou, self.exp_tailbeat[ind], s=0.5, c=color[ind], marker='.')
        # plt.legend(loc='upper left', markerscale=10)
        plt.legend(loc='best',labels=exp_ind)
        plt.axvspan(xmin=-1.5, xmax=0.0, facecolor='m', alpha=0.3)
        plt.axvspan(xmin=0.0, xmax=20.0, facecolor='y', alpha=0.3)
        plt.axvspan(xmin=20.0, xmax=40.0, facecolor='b', alpha=0.3)
        plt.savefig(result_path + '/tailbeat.png')

    def plot_tailbeatfudu(self, y, x, heading):
        n = len(self.exp_tailbeatfudu[0])
        fig_turn = plt.figure()
        ax5 = fig_turn.add_subplot(1, 1, 1)
        ax5.set_title("tail-beat amplitude")
        ax5.set_xlabel('hours')
        ax5.set_ylabel("mm")
        plt.xlim(-1.5, 40)
        # plt.ylim(0, y_zhou[5])
        x_zhou = np.linspace(-1.5, 40, n).tolist()
        for ind, data in enumerate(self.exp_tailbeatfudu):
            # print(color[ind])
            ax5.scatter(x_zhou, self.exp_tailbeatfudu[ind], s=0.5, c=color[ind], marker='.')
        # plt.legend(loc='upper left', markerscale=10)
        ax5.legend(loc='best',labels=exp_ind)
        plt.axvspan(xmin=-1.5, xmax=0.0, facecolor='m', alpha=0.3)
        plt.axvspan(xmin=0.0, xmax=20.0, facecolor='y', alpha=0.3)
        plt.axvspan(xmin=20.0, xmax=40.0, facecolor='b', alpha=0.3)
        plt.savefig(result_path + '/tailbeatfudu.png')
